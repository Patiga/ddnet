...
unsafe fn request_wgpu_backend_type(backend_type: i32, name: *mut *const std::ffi::c_char) -> bool {
    ...
    let backends = &[
        (c"WGPU (Vulkan)", wgpu::Backends::VULKAN),
        (c"WGPU (WebGPU)", wgpu::Backends::BROWSER_WEBGPU),
        (c"WGPU (GL)", wgpu::Backends::GL),
        (c"WGPU (Metal)", wgpu::Backends::METAL),
        (c"WGPU (DX12)", wgpu::Backends::DX12),
    ];
